...
This module replaces ``building2building/pipeline/regen_dataset.py`` and
``building2building/pipeline/scripts/regen_officemedium.sh`` (deleted in
this commit; see TODO.md § G4).  Phase M (M2)'s outstanding follow-up
redirects to G5: run this script with ``--building-type OfficeMedium``
instead of the old ``regen_officemedium.sh``.
...
Key design differences from ``regen_dataset.py``:
  ...
  5. Missing per-building artefact directories are a hard error (no silent
     skip); per AGENTS.md "fail loudly".
...
Phase G, item G4 (see TODO.md § G4).
"""
...
def rebuild_metadata_parquet(
    ...
    regenerated building (no silent skip; per AGENTS.md).
    """
