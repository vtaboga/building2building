...
Two canonical transfer axes ride on top of the normalized 3x2 task
...
* **Setpoint-mode transfer** (orthogonal to ``w_E``): train on
  ``task_occ_e05``, evaluate on ``task_const_e05`` or
  ``task_rand_e05``.  The two test tasks are *outside* the
...
The constructor's defaults (``train_task="task_occ_e05"``,
``test_task="task_occ_e0"``) implement the trade-off-transfer axis.
"""
...
class GoalAdaptation(BenchmarkProblem):
    ...
    Args:
        ...
        train_task: Named task preset for training.  Defaults to
            ``"task_occ_e05"`` (energy priced in, calibration regime).
        test_task: Named task preset for testing.  Defaults to
            ``"task_occ_e0"`` (comfort-only, calibration regime) -- the
            "trade-off transfer" axis.  See module docstring for the
            setpoint-mode-transfer axis.
    ...
    def __init__(
        ...
        split_index: int = 0,
        train_task: str = "task_occ_e05",
        test_task: str = "task_occ_e0",
