...
def get_net_conditioned_area(html_path: str) -> float | None:
    """
    ...
    mask = df.iloc[:, 0].astype(str).str.contains("Net Conditioned Building Area", case=False)
    if not mask.any():
        return None
...
def get_warmup_days(html_path: str) -> float | None:
