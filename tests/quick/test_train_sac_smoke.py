...
4. The ``reward_design_comparison_sac`` module imports correctly.
...
class TestSacModuleImports:
    ...
    def test_reward_design_sac_module_importable(self) -> None:
        import analysis.task_study.reward_design_comparison_sac as sac_mod

        assert hasattr(sac_mod, "run_training_sac")
        assert hasattr(sac_mod, "main")
        assert hasattr(sac_mod, "DEFAULT_TOTAL_TIMESTEPS")
        assert sac_mod.DEFAULT_TOTAL_TIMESTEPS == 1_000_000
        assert sac_mod.DEFAULT_EVAL_INTERVAL == 50_000
        assert sac_mod.DEFAULT_N_ENVS == 4

    def test_sac_module_reexports_ppo_surface(self) -> None:
        import analysis.task_study.reward_design_comparison_sac as sac_mod

        assert hasattr(sac_mod, "BUILDINGS")
        assert hasattr(sac_mod, "CheckpointData")
        assert hasattr(sac_mod, "TrainingSpec")
        assert hasattr(sac_mod, "make_plots")
        assert hasattr(sac_mod, "load_checkpoints")
        assert hasattr(sac_mod, "_decompose_episode")
